...
res=`cat ./Report/"$rtime"/result.txt |grep FPS|awk '{print $7wq}'|tail -n 1`
echo "glxgears_result is :"$res
refresh=`xrandr |grep "*"|awk '{print $2}'|awk -F* '{print $1}'`
echo "Current Screen Refresh Rate is:"$refresh

refresh_1=`echo "$refresh-10"|bc`
refresh_2=`echo "$refresh+10"|bc`

#echo $refresh_1
#echo $refresh_2

if [ $(echo "$res <= $refresh_2"|bc) = 1 ] && [ $(echo "$res >= $refresh_1"|bc) = 1 ];then
        echo "Turning ON Vsync takes effect!" > ./Report/"$rtime"/VsyncON.txt
else
        echo "Turning OFF Vsync takes effect!" > ./Report/"$rtime"/VsyncOFF.txt
fi
